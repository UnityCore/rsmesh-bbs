...
Type messages as you would from a handset (single-letter menu commands such as `B`, `M`, `R`). See the [User Guide](docs/RSMESH-BBS-USER-GUIDE.md) for menus and flows. Press Enter or type `X` for the main menu. Ctrl+C or Ctrl+D to quit.
...
See [docs/RSMESH-BBS-USER-GUIDE.md](docs/RSMESH-BBS-USER-GUIDE.md) for mesh menus, mail, bulletins, channels, and sysadmin capabilities.
